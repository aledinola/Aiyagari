...
% K is grid for current capital, K1 is grid for next-period capital, S is the 
%grid for the shock
C=(1+r)*K+w*S-K1;
ind=find(C>0);
U=repmat(-inf,size(C));
U(ind)=(C(ind).^(1-mu))/(1-mu);
...
R=cell(m,1);
for j=1:m
    %R{j}=feval(fname,repmat(s,1,n),repmat(z(j),n,n),repmat(s',n,1),varargin{:});
    % Using automatic expansion
    % s is nx1, z(j) is scalar, s' is 1xn ==> result is nxn and is stored
    % in the matrix R{j}
    R{j}=feval(fname,s,z(j),s',varargin{:});
end
R=vertcat(R{:}); % R is n*m,m
                 % if m=2, then R = [R1
                 %                   R2]
q=size(R,1);
I=speye(q);
